    ...
    entries.forEach((entry, index) => {
      const x =
        entry.yearStart == null
          ? 65
          : THREE.MathUtils.clamp((entry.yearStart - 1900) * 0.8 - 55, -70, 70)
      ...
      const color = new THREE.Color().setHSL(
        (realities.indexOf(entry.reality) * 0.137) % 1,
        0.75,
        0.62,
      )
...
export function TimelineCanvas({ entries }: { entries: readonly TimelineEntry[] }) {
  ...
  useEffect(() => {
    ...
    const scene = new TemporalLoomScene(host.current, entries)
  ...
  return (
    <div
      ...
      role="img"
      aria-label={`Three-dimensional temporal loom with ${entries.length} entries`}
    />
  )
}
