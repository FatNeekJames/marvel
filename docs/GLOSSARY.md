...
| **Timeline entry / temporal point** | One row in `TimelineEntry` — a single title/event/release with a year range, reality, and optional season/episode. 278 are seeded. |
...
| Term | Meaning |
| --- | --- |
| **Repository** | The `TimelineRepository` interface (contract) that UI/API code depends on, decoupling them from Prisma. |
| **Prisma adapter / `PrismaTimelineRepository`** | The production implementation of the repository using the Prisma client. |
| **Factory (`getTimelineRepository`)** | `lib/repositories/index.ts` function that constructs the repository wired to the database client. |
...
| **Prisma Postgres / `@prisma/dev`** | The dev-time local PostgreSQL-compatible server started by `scripts/dev.mjs`. |
...
| Script | What it runs |
| --- | --- |
| `npm run dev` | Orchestrated local dev: local DB + migrate + seed + Next dev. |
...
| `npm run db:deploy` | `prisma migrate deploy` (deploy environments). |
